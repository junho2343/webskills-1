class Path {
	...
	static getPath () {
		...
		Path.findPath(0,values.length,values,0);

		let html = ``;
		Path.minPath.forEach((x, idx) => {

			if(idx != 0){
				html += ` => ${Path.normal[ idx-1 ][ x ]}분`;
			}

			html += `(장소 : ${Path.lbl[ x ]})`;
		})

		let template = Path.template.replace("{{path}}",html).replace("{{cost}}",Path.min);
		$(".shortest-path").html(template);

	}

	static findPath (level, max, arr, dis) {
		if(Path.min < dis) return;
		...
		for(let i = 0; i < arr.length; i++){
			...
			if(level != 0){
				tempDis = dis + Path.normal[ Path.path[level-1] ][ Path.path[level] ];
			}

			Path.findPath(level+1, max, tempArr, tempDis);
		}
